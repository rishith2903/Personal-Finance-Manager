    ...
    const loadSpendingData = async () => {
        ...
        try {
            ...
            for (let i = 5; i >= 0; i--) {
                ...
                monthlyStats.push({ month: monthLabel, income: monthIncome, expense: monthExpense });
            }
                ...
                <div className="h-52">
                    <div className="flex items-end justify-between h-44 gap-2 px-2">
                        {monthlyData.map((data, index) => {
                            const incomeHeight = data.income > 0 ? Math.max((data.income / maxAmount) * 100, 10) : 2;
                            const expenseHeight = data.expense > 0 ? Math.max((data.expense / maxAmount) * 100, 10) : 2;
                            return (
                                <div key={index} className="flex-1 flex flex-col items-center">
                                    <div className="w-full flex gap-1 items-end h-full">
                                        <div
                                            className="flex-1 bg-gradient-to-t from-emerald-500 to-emerald-400 rounded-t-md transition-all hover:opacity-80 cursor-pointer relative group"
                                            style={{ height: `${incomeHeight}%`, minHeight: data.income > 0 ? '24px' : '4px' }}
                                        >
                                            <div className="absolute -top-7 left-1/2 -translate-x-1/2 bg-gray-800 text-white text-xs py-0.5 px-1.5 rounded opacity-0 group-hover:opacity-100 whitespace-nowrap z-10">
                                                ₹{data.income.toFixed(0)}
                                            </div>
                                        </div>
                                        <div
                                            className="flex-1 bg-gradient-to-t from-red-400 to-red-300 rounded-t-md transition-all hover:opacity-80 cursor-pointer relative group"
                                            style={{ height: `${expenseHeight}%`, minHeight: data.expense > 0 ? '24px' : '4px' }}
                                        >
                                            <div className="absolute -top-7 left-1/2 -translate-x-1/2 bg-gray-800 text-white text-xs py-0.5 px-1.5 rounded opacity-0 group-hover:opacity-100 whitespace-nowrap z-10">
                ...
                <div className="space-y-5">
                    ...
                    <div className="p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg border border-blue-100">
                        <div className="flex items-center justify-between">
                            ...
                            <span className={`text-xl font-bold ${incomeVsExpense.income - incomeVsExpense.expense >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
                                {incomeVsExpense.income - incomeVsExpense.expense >= 0 ? '+' : ''}₹{(incomeVsExpense.income - incomeVsExpense.expense).toFixed(2)}
                            </span>
                        </div>
                        <p className="text-xs text-gray-500 mt-1">
                            {incomeVsExpense.income - incomeVsExpense.expense >= 0 ? "Great! You're saving money." : "Spending more than earning."}
                        </p>
                    </div>
                </div>
            </div>
